...
class AutoEncoder():
    ...
    def load(cls, save_folder="."):
        ...
        for i in range(2):
            ...
            except FileNotFoundError as e: save_folder = "trained_models"/Path(temp)
    ...
    def train_on_batch(self, batch_size, num_epoch):
        ...
        for epoch_nr in range(0, num_epoch):
            ...
            if epoch_nr%2 == 0:
                self.save(f"{self.name}-{epoch_nr}-{round(meanloss,5)}-{round(meanloss_val,5)}")
                pass
            self.loss = []
            self.val_loss_m = []
            count_val = 0
    ...
    def _add_bottle_neck(self, model):
        ...
        model = Flatten()(model)
        model = Dense(self.latent_space_dim, name="encoder_output",kernel_regularizer=self.regularizer,use_bias=False)(model)#regularizers.l2(1e-7))(model)  # dimensionality of latent space -> outputshape
    ...
    def _add_dense_layer(self, decoder_input):
        ...
        dense_layer = Dense(num_neurons, name="decoder_dense",kernel_regularizer=self.regularizer,use_bias=False)(decoder_input)#regularizers.l2(1e-7))(decoder_input)
        return dense_layer
    ...
    def _add_decoder_output(self, x):
        ...
        x = conv_transpose_layer(x) 
        x = BatchNormalization(name=f"decoder_bn_{self._num_conv_layers}")(x)
        x = ReLU(name=f"decoder_relu_{self._num_conv_layers}")(x)
        ...
        conv_layer = Conv2D(
            ...
            strides=(1,1),
            padding="same",
            name=f"decoder_conv_layer_final",
            kernel_initializer=self.weight_initializer,
            kernel_regularizer=self.regularizer
        )
