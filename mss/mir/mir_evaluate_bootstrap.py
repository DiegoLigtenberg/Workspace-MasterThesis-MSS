...
class Conv_Statistics():
    ...
    def calculate_m_auc(self,y_test,preds,model_name,save_ROC):
        ...
        print("rare",y_test.shape,preds.shape)
        ...
        for i in range(11):
            ...
            for nr in range(1000):
                ...
                idx = np.random.randint(0, y_test.shape[0], size=y_test.shape[0])
                fpr, tpr, tresholds = metrics.roc_curve(y_test[idx, i], preds[idx, i], drop_intermediate=True)
                auc = metrics.auc(fpr, tpr)
                auc_values.append(auc)
                aucs.append(auc)
            plt.hist(auc_values,bins=30)
            # plt.show()
            # print(len(auc_values))
            print("error", np.round( np.percentile(auc_values,(2.5,97.5))[1] - np.percentile(auc_values,(2.5,97.5))[0] ,3))
            # print(np.percentile(auc_values,(2.5,97.5)))
            # print(np.std(auc_values))
            auc_values = []
