...
class Conv_Statistics():
    ...
    def _find_optimal_cutoff(self,y_test,preds,fpr,tpr,tresholds,optimal_tresholds,i):
        ...
        for j,duo in enumerate(conc):
            ...
            v1 = np.array([0,1])  # (fpr, tpr) lowering the 1st value makes the model more strict in selecting ! 
            v2 = np.array(duo)
            v3 = v1-v2
...
class Evaluate():
    def __init__(self,models,model):
        ...
        self.statistics = Conv_Statistics(verbose=False)
...
evaluate = Evaluate(["with_postprocessing"],model="with_postprocessing")
