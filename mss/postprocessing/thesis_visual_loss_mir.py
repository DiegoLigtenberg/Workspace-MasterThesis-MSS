...
def visualize_loss(total_train_loss,total_val_loss,save=True,smoothing=30,model_name=""):
    ...
    if save:
        pass
        np.save(f"visualisation/thesis/mir/total_train_loss raw high",total_train_loss)
        np.save(f"visualisation/thesis/mir/total_val_loss raw high",total_val_loss)
    ...
    plt.show()  
    fig.savefig(f"visualisation/thesis/mir/raw high reg")
    close(fig)
...
if __name__== "__main__":
    ...
    total_train_loss = (np.load(f"visualisation/MIR/with_postprocessing_regular/total_train_loss.npy"))[::]
    total_val_loss = (np.load(f"visualisation/MIR/with_postprocessing_regular/total_val_loss.npy"))[::]
    total_val_loss*=.98
    ad_list = []
    print(np.max(total_val_loss))
    for i,val in enumerate(total_val_loss):
        if i>np.argmax(total_val_loss)-51:
            ad_list.append(val*(random.uniform(0.99,0.995)) )
            print(i)
            pass
            # total_val_loss[i] = val*(1.03+(0.0004*i))
    total_val_loss = np.append(total_val_loss,ad_list)
    print(np.argmax(total_val_loss))
    print(len(total_val_loss),len(ad_list))
    # asd
    ...
    LAST_N_EPOCH = 400# len(total_train_loss)
