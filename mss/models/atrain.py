...
def train(learning_rate,batch_size,epochs,model_name=""):
  ...
  variatonal_auto_encoder = AutoEncoder(
      ...
      conv_kernels=(5,   5,   5,   5,   5,   5), # KERNEL SIZE SHOULD BE DIVISIBLE BY STRIDE! but only when upsampling! -> OTHERWISE CITY BLOCK PATTERN -> receptive field
...
def main():
    ...
    # for i in range(1):
    #   variational_auto_encoder = AutoEncoder.load(MODEL_NAME) 
    #   variational_auto_encoder.name=MODEL_NAME
