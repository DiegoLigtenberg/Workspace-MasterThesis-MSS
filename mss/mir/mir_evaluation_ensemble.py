...
if __name__ == "__main__":
    ...
    #postprocessing can also be used on base
    ...
    x_train, y_train = dataloader.load_data(dataset="train", model="base")
    x_test, y_test = dataloader.load_data(dataset="test", model="base")
    # x_train = x_train[::5]
    # y_train = y_train[::5]
    ...
    model_name = "mir_model_no_postprocessing_improved" # 100 epoch
    
    conv_net = existing_model(model_name)
    dataloader = MIR_DataLoader()
    x_train, y_train = dataloader.load_data(dataset="train", model="no_postprocessing")
    x_test, y_test = dataloader.load_data(dataset="test", model="no_postprocessing")
    # x_train = x_train[::5]
    # y_train = y_train[::5]
    ...
    # '''
    preds2 = conv_net.model.predict(my_test_batch_generator)

    model_name = "mir_model_with_postprocessing_ auc 0.0_00 auc 0._1_1_1_59 auc 0.745" # 100 epoch
    ...
    # '''
    preds3 = conv_net.model.predict(my_test_batch_generator)
    ...
    for i in range(11):
        ...
        for j,duo in enumerate(conc):
            ...
            v1 = np.array([-1.0,1]) # (fpr, tpr) lowering the 1st value makes the model more strict in selecting ! -> reduces missclassification but also reduces 'hits' -> predict 0 more often
            v2 = np.array(conc[j])
            v3 = np.abs(v1-v2)
